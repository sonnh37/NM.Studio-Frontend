...
export default function SubCategoryTable() {
  ...
  const queryParams = useMemo(() => {
    const params: SubCategoryGetAllQuery = useQueryParams(
      formValues,
      columnFilters,
      pagination,
      sorting
  ...
  const { data, isFetching, error } = useQuery({
    ...
    queryFn: () => subCategoryService.getAll(queryParams),
    enabled: shouldFetch,
  ...
  return (
    <DataTableComponent
      ...
      deletePermanentFunc={(command) => subCategoryService.delete(command)}
      updateUndoFunc={(command) => subCategoryService.update(command)}
      table={table}
      queryKey={query_key}
    >
      <DataTableToolbar
